...
class AILogicTest {
	...
	void testChooseTerritoryToInitialClaim() {
		...
		assertTrue(tmp == CountryName.Kamchatka 
				|| tmp == CountryName.NorthwestTerritory 
		...
		assertTrue(tmp == CountryName.Alaska 
				|| tmp == CountryName.CentralAmerica
	...
	void testChooseTerritoryToInitialReinforce() {
		for(Territory t : territories.values()) {
			if(!(t.getCountryName() == CountryName.Ukraine
					|| t.getCountryName() == CountryName.Siberia
		...
		assertTrue(tmp == CountryName.Ukraine 
				|| tmp == CountryName.Siberia
				|| tmp == CountryName.Brazil);
	}
	...
	void testChooseTerritoryToReinforce() {
		...
		assertTrue(tmp.getKey() == CountryName.Brazil
				&& tmp.getValue().equals(5));
	}
...
//		territories.get(CountryName.EasternAustralia).setOwnedByPlayer(player2);
		...
		assertTrue(tmp.getKey() == CountryName.Ukraine
				&& tmp.getValue() == CountryName.MiddleEast); 
	}
	...
	void testChooseTerritoriesPairFortify() {
		...
		assertTrue(tmp.getKey() == CountryName.Brazil
				&& tmp.getValue() == CountryName.Mongolia);
	}
	...
	void testGetRiskCardsTurnIn() {
		...
		Card card4 = new Card(CountryName.Alaska, 3, Parameter.territoryPNGdir + "northamerica-alaska.png");
		Card card5 = new Card(CountryName.CentralAmerica, 4, Parameter.territoryPNGdir + "northamerica-centralamerica.png");
		Card card7 = new Card(CountryName.Brazil, 5, Parameter.territoryPNGdir + "southamerica-brazil.png");
		...
		for(String s : tmp) {
			if(s.equals("Joker")) {
				...
				list.add(new Card(CountryName.valueOf(s), i, Parameter.territoryPNGdir + "africa-congo.png")); // country pic is here irrelevant
			}
			i++;
		}
		...
		assertTrue((list.stream().allMatch(o -> o.getCardSymbol() == list.get(0).getCardSymbol()))
		        || (list.stream().map(Card::getCardSymbol).distinct().collect(Collectors.toSet())
		                ...
		                || (list.stream().map(Card::getCardSymbol).reduce(0,
		                    (a, b) -> a + b) < 0)));

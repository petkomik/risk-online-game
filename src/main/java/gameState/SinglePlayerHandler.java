...
/**
 * This class connects the GUI and GameHandler for Offline Games. Takes user
 * input from GUI. Takes decision from GameHandler and changes GUI.
 *
 * @author pmikov
 *
 */
...
public class SinglePlayerHandler {
    ...
    public SinglePlayerHandler(Lobby lobby, GamePaneController gamePaneController, GameType gameType) {
    ...
    /**
     * Called from GUI. Player decides to close choose number of troops pane.
    ...
    /**
     * Called from GUI. Player confirms choice for number of troops in choosing
     * number of troops pane.
     *
     * @param country    CountryName of territory attacked or sending troops to
     * @param choosePane ChoosePane enum for the pane
    ...
    /**
     * Called from GameHandler. Throwing Battle dice is confirmed. Sending new Dice
     * Values and updated number of troops.
     *
    ...
    /**
     * Called from GameHandler. Confirms roll intiial dice and gives the value of
     * the dice.
     *
    ...
    /**
     * Called from GameHandler. Sets the the given period on the GUI. Based on the
     * period the bottom pane displays different info / buttons.
     *
    ...
    /**
     * Called from GameHandler. Sets the the given phase on the GUI. Based on the
     * phase the bottom pane displays different info / buttons.
     *
    ...
    /**
     * Called from GameHandler. Paint the country in the color of player with id and
     * places one troop on it.
     *
    ...
    /**
     * Called from GameHandler. Changes the current player on the GUI by swapping to
     * the correct color and avatar, and setting the corrct number of troops
     *
    ...
    /**
     * Called from GameHandler. Changes the player which is playing from the
     * instance.
     *
    ...
    /**
     * Called from GameHandler. Opens Chooosing Troops Pane, where the player
     * decides on how many troops to deploy / move / attack with.
     *
    ...
    /**
     * Called from GameHandler. Moves troops from one territory toanother on GUI and
     * updates their number of troops.
     *
    ...
    /**
     * Risk cards have been turned in scuccessfully. Update Cards avaible for player
     * on GUI and troops left to deploy.
     *
    ...
    /**
     * Sets Territories disabled. And "Points up" a country. Used during fortifying
     * and attacking.
     *
     ...
     *
     * @param ranks Array with the ranks of player in the order of Players List in
     *              the Lobby.
     */
